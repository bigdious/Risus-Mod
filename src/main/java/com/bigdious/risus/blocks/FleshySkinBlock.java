...
public class FleshySkinBlock extends ActuallyUseableDirectionalBlock {
	...
	public ItemInteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult result) {
		if (stack.is(RisusItems.ORGANIC_MATTER.get())) {
			...
			ServerParticleUtils.spawnParticlesOnBlockFace(level, pos, new ItemParticleOption(ParticleTypes.ITEM, new ItemStack(Blocks.GRAY_CONCRETE)), UniformInt.of(1, 4), state.getValue(FACING), () -> new Vec3(Mth.nextDouble(level.getRandom(), -0.1, 0.1), Mth.nextDouble(level.getRandom(), -0.1, 0.1), Mth.nextDouble(level.getRandom(), -0.1, 0.1)), 0.6);
