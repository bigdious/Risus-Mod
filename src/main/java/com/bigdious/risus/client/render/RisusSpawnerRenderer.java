...
public class RisusSpawnerRenderer implements BlockEntityRenderer<RisusSpawnerBlockEntity> {
	...
	public void render(RisusSpawnerBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
	...
	public static void renderEntityInSpawner(float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight, Entity entity, EntityRenderDispatcher entityRenderer, double oSpin, double spin) {
		...
		entityRenderer.render(entity, 0.0F, 0.0F, 0.0F, 0.0F, partialTick, poseStack, buffer, packedLight);
		poseStack.popPose();
	}

	public AABB getRenderBoundingBox(RisusSpawnerBlockEntity blockEntity) {
		BlockPos pos = blockEntity.getBlockPos();
		return new AABB(pos.getX() - 1.0F, pos.getY() - 1.0F, pos.getZ() - 1.0F, pos.getX() + 2.0F, pos.getY() + 2.0F, pos.getZ() + 2.0F);
	}
}
