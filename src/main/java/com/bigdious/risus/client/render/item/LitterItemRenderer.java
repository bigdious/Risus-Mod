...
public class LitterItemRenderer extends BlockEntityWithoutLevelRenderer {
	...
	public void renderByItem(ItemStack stack, ItemDisplayContext context, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
		if (stack.getItem() instanceof LitterItem) {
			Entity entity = EntityCache.fetchEntity(RisusEntities.LITTER.get(), Minecraft.getInstance().level, stack.getOrDefault(RisusDataComponents.BLOCK_STATE, Blocks.AIR.defaultBlockState()));
			if (entity != null) {
