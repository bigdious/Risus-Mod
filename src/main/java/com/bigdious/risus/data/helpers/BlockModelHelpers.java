...
import static com.bigdious.risus.Risus.prefix;
...
public abstract class BlockModelHelpers extends BlockStateProvider {
	...
	public BlockModelHelpers(PackOutput output, ExistingFileHelper exFileHelper) {
		super(output, Risus.MODID, exFileHelper);
	}
	...
	protected void plankBlocks(String variant, Block plank, Block slab, StairBlock stair, Block button, Block fence, Block gate, Block plate, DoorBlock door, TrapDoorBlock trapdoor) {
		this.plankBlocks(variant, plank, slab, stair, button, fence, gate, plate, door, trapdoor, false, false);
	}

	protected void plankBlocks(String variant, Block plank, Block slab, StairBlock stair, Block button, Block fence, Block gate, Block plate, DoorBlock door, TrapDoorBlock trapdoor, boolean cutoutDoors, boolean correctDoors) {
	...
	protected void bisectedStairsBlock(DeferredHolder<Block, StairBlock> block, String name, ResourceLocation side, ResourceLocation end, ResourceLocation middle) {
		ModelFile stairs = this.models().withExistingParent(name, Risus.prefix("block/util/bisected_stairs")).texture("side", side).texture("end", end).texture("middle", middle);
		ModelFile stairsInner = this.models().withExistingParent(name + "_inner", Risus.prefix("block/util/bisected_inner_stairs")).texture("side", side).texture("end", end).texture("middle", middle);
		ModelFile stairsOuter = this.models().withExistingParent(name + "_outer", Risus.prefix("block/util/bisected_outer_stairs")).texture("side", side).texture("end", end).texture("middle", middle);
